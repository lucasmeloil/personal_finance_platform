...
export const list = query({
  args: {
    status: v.optional(v.union(v.literal("pending"), v.literal("received"), v.literal("overdue"))),
  },
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const accountId = await ctx.db.insert("accountsReceivable", {
      ...args,
      status,
      userId,
    });
...
export const update = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const today = new Date().toISOString().split('T')[0];
    const status = account.status === "received" ? "received" : 
...
export const markAsReceived = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const receivedDate = args.receivedDate || new Date().toISOString().split('T')[0];

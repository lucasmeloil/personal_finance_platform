...
export function AccountsPayable() {
  ...
  const accounts = useQuery(
    api.accountsPayable.list,
    statusFilter === "all" ? {} : { status: statusFilter }
  ) || [];
  const people = useQuery(api.people.list) || [];
  const createAccount = useMutation(api.accountsPayable.create);
  const updateAccount = useMutation(api.accountsPayable.update);
  const markAsPaid = useMutation(api.accountsPayable.markAsPaid);
  const removeAccount = useMutation(api.accountsPayable.remove);
  ...
  const handleMarkAsPaid = async (id: Id<"accountsPayable">) => {
    try {
      await markAsPaid({ id });
  ...
  const handleDelete = async (id: Id<"accountsPayable">) => {
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        {[
          ...
          { key: "paid", label: "Pagas", count: accounts.filter(a => a.status === "paid").length },
        ].map((filter) => (
          <button
            key={filter.key}
      ...
      <div className="space-y-3">
        {accounts.map((account) => (
          <AccountCard
            ...
            onMarkAsPaid={() => handleMarkAsPaid(account._id)}
...
function AccountCard({ account, onEdit, onMarkAsPaid, onDelete }: any) {
          ...
          <div className="flex-1">
            ...
            {account.paidDate && (
              <div className="mt-2 text-sm text-green-600">
                <span className="font-medium">Pago em:</span> {formatDate(account.paidDate)}
              </div>
            )}
          </div>
      ...
      <div className="modal-content">
        <div className="card-header">
          <h3 className="text-lg font-semibold">
            {account ? "Editar Conta" : "Nova Conta a Pagar"}
          </h3>
        </div>
